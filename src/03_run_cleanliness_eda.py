...
def profile_cleanliness():
    raw_file = "data/raw/daily-emissions-f436a512-59f3-466a-9dd4-bc31cafbbb3a.csv"
    cleaned_file = "data/processed/cleaned_daily_emissions.csv"
    ...
    print("Generating cleanliness chart...")
    ...
    plt.title('Data Quality Audit: Missing Values Comparison (Raw vs Cleaned)', fontsize=14, fontweight='bold')
    plt.xticks(x, [f"{i}: {name}" for i, name in zip(df_compare['index'], df_compare['cleaned_feature'])], rotation=90, fontsize=8)
    plt.legend()
    plt.grid(axis='y', linestyle='--', alpha=0.5)
    plt.tight_layout()
    plt.savefig("reports/plots/missing_values_comparison.png", dpi=300)
    plt.close()
    
    # 5. Write Report
    print("Writing Cleanliness Report...")
    os.makedirs("reports", exist_ok=True)
    with open("reports/Cleanliness_Report.md", "w", encoding="utf-8") as f:
        f.write("# Data Cleanliness & Profiling Report (Before vs After)\n\n")
        f.write("This report provides a detailed comparison of the data quality (cleanliness) of the raw daily emissions dataset versus the cleaned dataset.\n\n")
        ...
        f.write("![Missing Values Comparison](plots/missing_values_comparison.png)\n")
        
    print("Cleanliness EDA report generated successfully!")
